...
removeButton.addEventListener("click", ()=> {
    ...
    const removetask = document.getElementsByClassName("removetask")[0];
...
removeday.addEventListener("change", showlist);

function showlist(){
    const tasklist = document.getElementById("tasklist");
    daytasks = JSON.parse(localStorage.getItem(removeday.value));
    for (let i = 0; i < daytasks.length; i++) {
        ...
        taskdiv.style.backgroundColor="black";
        taskdiv.style.boxShadow="0px 0px 0px cyan";
        taskdiv.style.border="3px solid cyan";
        taskdiv.style.position="absolute";
        taskdiv.style.borderRadius="3vw";
        taskdiv.style.left = "5%";
        taskdiv.style.top = `${i*25}%`;
        taskdiv.style.width='90%';
        taskdiv.style.height="20%";
        taskdiv.innerHTML=`<p class="intext" style="left 40%;up:30%;">${taskname}</p>`
        taskdiv.innerHTML+=`<p class="intext" style="left 40%;down"30%">${taskstart} - ${taskend}</p>`
        taskdiv.innerHTML+=`<button class="remove" style="position:absolute;top:17.5%;right:15%;width:8%;height:65%;background-color:rgb(0,0,0,0);border-style:solid;border-radius:100px;border-color:white;">❌</button>`    
        deleteButton = taskdiv.getElementsByClassName("remove")[0];
        deleteButton.addEventListener("click", ()=> {
            daytasks.splice(i, 1);
            localStorage.setItem(removeday.value, JSON.stringify(daytasks));
